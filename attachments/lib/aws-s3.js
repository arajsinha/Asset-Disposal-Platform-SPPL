...
module.exports = class AWSAttachmentsService extends require("./basic") {
  ...
  async get(attachments, keys, req = {}) {
    const response = await SELECT.from(attachments, keys);
    if (response?.url) {
...
async updateContentHandlerNonDraft(req, target, next) {
  if (req?.data?.content) {
    const response = await SELECT.from(target, { ID: req.data.ID }).columns("url");
    if (response?.url) {
      const Key = response.url;
      const input = {
        Bucket: this.bucket,
    ...
    srv.prepend(() => {
      if (mediaElement.drafts) {
        ...
        srv.on(
          "PUT",
          mediaElement,
          this.updateContentHandlerNonDraft.bind(this)
        );
      }
    });
  }
