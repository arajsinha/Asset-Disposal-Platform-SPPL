...
module.exports = class AttachmentsService extends cds.Service {
  ...
  draftSaveHandler(attachments) {
    ...
    return async (_, req) => {
      ...
      let draftAttachments = await SELECT(queryFields)
      ...
      if (!draftAttachments.length) {
        draftAttachments = await SELECT(queryFields)
          .from(attachments)
          .where([
            ...req.subject.ref[0].where.map((x) =>
              x.ref ? { ref: ["up_", ...x.ref] } : x
  ...
  SaveHander(attachments) {
    const queryFields = this.getFields(attachments);
    return async (_, req) => {
      // The below query loads the attachments into streams
      const attachmentsLocal = await SELECT(queryFields)
        .from(attachments)
        .where([
          ...req.subject.ref[0].where.map((x) =>
            x.ref ? { ref: ["up_", ...x.ref] } : x
      ...
      if (attachmentsLocal.length)
        await this.put(attachments, attachmentsLocal);
    };
  }
